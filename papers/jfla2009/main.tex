...
\path[use as bounding box] (0,0) node[draw] (a) {comme ceci};
...
abcisses $0cm$ pour $A$ et $2cm$ pour $B$. Mais si vous changez d'avis
sur le contenu de $A$ et que  la taille de la boîte change, $A$ risque
alors de se superposer à $B$. Il faut alors replacer toutes les boîtes
de votre  figure. Pour éviter  ça, \mlpost\ propose  diverses méthodes
pour  placer les  boîtes \emph{les  unes par  rapport aux  autres}. On
gagne alors du temps lors de  la création et lors des modifications de
la figure.
...
Le choix de la représentation symbolique de la plupart des objets impose
...
syntaxe abstraite (AST) contient beaucoup de n\oe uds idéntiques mais
...
que la comparaison efficace fourni par ce dernier). Quand on passe un
...
Le n\oe ud $f$ est réellement utilisé deux fois, alors que le n\oe ud
$e$ n'est utilisé qu'un seule fois, par le n\oe ud $d$, même si celui-ci
est utilié deux fois à son tour.
...
l'utilisateur peut créer ses propres types de flèche.
