...
Par souci de complétude, nous devons aussi mentionner
\asymptote\cite{asymptote}, également un langage dédié à la création de
figures. Par la suite, nous ne considérons plus les langages dédiés, nous
allons donc pas rentrer dans les détails de cet outil.
...
%TODO: code de loop + illustration de son fonctionnement
...
Le but de cette section est de montrer qu'il est très facile d'écrire sa
propre mini-bibliothèque qui utilise \mlpost\ afin de réaliser ses figures.
...
d'implémenter une fonction qui dessine un rectangle correspondant à une classe :
\begin{ocaml}
...
rectangle dans le diagramme (l'argument labelisé à \texttt{Box.vblock})
et comme titre du rectangle créé. Les attributs et les méthodes sont alignés
...
Ici, on a d'abord créé deux rectangles de classe avec la fonction
\texttt{classblock}. Ces rectangles sont ensuite alignés verticalement, et une
...
figure~\ref{fig:archi}. Au niveau le plus bas se trouvent les
interfaces correspondant aux types primitifs de \metapost. Ce sont ces
objets qui sont {\it in fine} compilés en du code \metapost, et nous
les décrivons de manière plus détaillée dans la
section~\ref{subsec:types}. La couche intermédiaire contient des
éléments que nous estimons être de bas niveau mais qui ne sont pas
présents dans \metapost: ils sont propres à \mlpost et ont été
construits à partir de la couche inférieure. Les
sections~\ref{subsec:boxes} et \ref{subsec:arrows} reviennent plus en
détail sur deux de ces modules, respectivement \texttt{Box} et
\texttt{Arrow}.
...
abstraite pour profiter au maximum du partage probable entre les
différents objets. % TODO expliquer plus

Ceci diminue l'utilisation de la mémoire, mais ne change rien à la
taille des fichiers générés. La structure hash-consée {\em réalise} le
partage, mais elle ne sait pas quels sont les n\oe uds en effet utilisés
au moins deux fois. Pour cela, il suffit de traverser la structure une
fois en entier, en comptant le nombre de fois qu'on a passé chaque n\oe
ud (cette information peut-être stockée efficacement dans une table,
à l'aide de la valeur de hash calculée pour chaque n\oe ud pour le
hash-consing ainsi que la comparaison efficace fournie par ce dernier).
Quand on passe un n\oe ud déjà rencontré, on ne considère pas ses
sous-n\oe uds. Il est néanmoins possible de rencontrer une nouvelle fois
un sous-n\oe ud d'une structure, comme le montre l'arbe ci-dessous :
...
Après cette analyse, la génération du fichier \metapost\ devient très
simple : il suffit de traverser l'AST encore une fois, et quand on passe par
un n\oe ud qui a été utilisé au moins deux fois, on lui attribue une variable
et on ajoute une définition correspondante dans le code \metapost. De cette
façon, on arrive à avoir du code \metapost\ relativement petit, malgré la
délégation des calculs à \metapost.

% TODO : des chiffres pour justifier "relativement petit" ?
% TODO ajouter explications sur les contraintes syntaxiques de \metapost
...
l'utilisateur peut créer ses propres types de flèches.

Le module \texttt{Arrow} dispose de trois types:
\begin{itemize}
\item me type abstrait \texttt{body}  décrit le corps des flèches, qui
  peuvent être  composées de plusieurs traits parallèles  ou en série,
  avec chacun leur style propre;
...
\item le type abstrait \texttt{kind} décrit une  catégorie de flèche.
  Il s'agit de la combinaison d'un  corps et de zéro, une ou plusieurs
  têtes.
\end{itemize}

Pour  dessiner une flèche,  on utilise  la fonction  \texttt{draw} qui
permet de  dessiner une  flèche d'une catégorie  donnée en  suivant un
chemin donné.
