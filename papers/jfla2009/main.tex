...
\subsection{Autres applications : arbres et graphes de fonctions}

% arbre -- JCF

% plot -- SL
Un autre exemple d'application que nous avons développée au sein
d'\mlpost{} est \texttt{Plot}, qui permet de tracer des graphes de
fonction. La figure suivante montre un exemple très simple
...
cannevas de 4 unités sur 2, qui est l'objet de base de l'extension
...
L'exemple suivant, réalisé par Florence Plateau,
...
\paragraph{Exemples utilisant des calculs en \ocaml} % a dire mieux
% exemples utilisant des calculs en Caml
L'exemple qui conclut le paragraphe précédent est en fait intéressant
à plus d'un titre. Il nous permet en effet d'illustrer un des
avantages de \mlpost: la capacité de dessiner directement un objet que
l'on a calculé/programmé en \ocaml.  

Ainsi les fonctions dans l'exemple ci-dessus ont été codées comme des
...
$f_{\omega_2}$, la ligne marquera alors toujours les discontinuités
correctement sans avoir à la mettre à jour manuellement. Cela est
également vrai pour certains labels de la figure, comme l'abscisse
$[\omega_2]_5$. Ceci offre une flexiblité très intéressante lors de la
phase de développement d'une telle figure.

Un autre exemple c'est Bresenham...
% bresenham -- JCF

% \paragraph{Exemples du monde réel}
% je sais tjs pas quoi faire de cette figure de yannick
\begin{center}
...
fichier \postscript\ de sortie, \verb|figure.1|\footnote{ce fichier
  n'a pas le suffixe .ps puisque ce n'est pas un fichier \postscript
  proprement dit : il manque l'en-tête.}. Pour inclure cette figure
dans un document \LaTeX, il suffit d'utiliser la commande
\verb|\includegraphics| du package \textsf{graphicx}.

Nous espérons avoir convaincu le lecteur des avantages que la
présentation sous forme de bibliothèque apporte : familiarité avec le
langage pour les programmeurs \ocaml, typage fort, des
construction de programmation de haut-niveau etc.  Mais ce choix
implique également que le fichier \metapost\ de sortie ne sera pas
obtenu par une {\em compilation} du code source proprement dite, mais
par une exécution du programme qui construit un arbre de syntaxe
abstraite (AST). Cette méthode a l'inconvénient qu'une boucle ou
...
aussi les boucles dans les fichiers \postscript\ de sortie.
