...
propre extension fondée sur \mlpost\ afin de réaliser ses figures.
...
bibliothèque \ocaml. Nous espérons avoir convaincu le lecteur des avantages
que cela apporte : familiarité avec le langage pour ceux qui connaissent
\ocaml, typage fort, des construction de programmation de haut-niveau etc.
Mais ce choix implique également que le fichier \metapost\ de sortie ne sera
pas obtenu par une {\em compilation} du code source proprement dite, mais par
une exécution du programme qui construit un arbre de syntaxe abstraite (AST).
Cet arbre est ensuite imprimé dans un fichier et interprété par \metapost.
Cela a l'inconvénient qu'une boucle ou itération dans le programme de départ
sera traduite par une suite de commandes obtenues par le déroulement de la
boucle, et non par une construction de boucle du langage cible. Ceci étant
dit, dans notre cas, le coût supplémentaire est faible, puisque \metapost\
déroule aussi les boucles dans les fichiers \postscript\ de sortie. 
...
objets qui sont {\it in fine} traduits en du code \metapost, et nous
...
simple : il suffit de traverser l'AST encore une fois, et quand on passe
par un n\oe ud qui a été utilisé au moins deux fois, on lui attribue
une variable et on ajoute une définition correspondante dans le code
\metapost. Il faut néanmoins prendre en compte les particularités
syntaxiques de \metapost\ telles que la précédence inhabituelle des
opérateurs arithmétiques et la restriction de l'application de certaines
constructions à des variables. De cette façon, on arrive à avoir du
code \metapost\ relativement petit, malgré la délégation des calculs à
\metapost.
