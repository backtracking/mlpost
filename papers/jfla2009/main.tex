...
d'un coutour, qui peut être effectivement tracé ou non. On peut
...
modifications de la figure.  L'exemple suivant montre la même figure
qu'avant, mais en utilisant l'alignement horizontal \verb|hbox|
(l'argument optionnel \texttt{padding} permet de spécifier
l'espacement horizontal entre deux boîtes):
...
Un autre choix que nous avons fait est celui de la persistance :
...
inchangés, d'être préservée et encore accessible. Cela a l'avantage de
pouvoir réutiliser plus facilement une boîte à plusieurs endroits
différents du dessin, avec des attributs différents. En déplacant une
boîte, on en a créé une nouvelle et laissé l'ancienne intacte à sa
place.
...
Dans cette section, nous montrons quelques petits exemples de développement de
dessin en \mlpost. 
...
\end{ocaml}
Le paramètre \texttt{stroke} permet de spécifier si (et dans quelle
couleur) le contour doit être tracé. De manière générale, les
...
% plot -- SL
Un autre exemple d'extension que nous avons développée au sein
...
fonctions) qui permettent de réaliser des figures plus compliquées.
