...
La couche bas-niveau de \mlpost est une interface fidèle à \metapost. Elle
comporte donc tous les types de base de \metapost :

\begin{description}

  \item[Le type numérique (module \texttt{Num})] représente des longeurs.
    ...
    pas connues à l'execution.  La plupart des calculs sont donc effectués de
  ...
  \item[Le type couple ou pair (module \texttt{Point})] représente des points
    ...
    numériques, les points ne sont pas simplement des pairs de floats, mais
  ...
  \item[Les chemins (module \texttt{Path})] sont des lignes représentés par
    ...
    interfacés. On peut dessiner des lignes droites ou des lignes courbes en
    précisent les angles de départ et arrivée, la tension de la courbe, etc.
...
Cette partie bas-niveau de \mlpost\ présente néanmoins quelques difficultés.
Premièrement, la plupart des modules présentés sont à priori mutuellement
récursifs : par exemple, les transformations s'appliquent à tous les autres
objets, donc chaque module contient une fonction
\begin{ocaml}
...
existent aussi entre types et sont aggravés par la representation symbolique
...
réference à ce fichier. Par exemple, dans le fichier \texttt{path.ml}, qui
...
avons écrit un fichier /texttt{mlpost.mli} qui contient les définitions des
...
faire réference à \texttt{Path} et inversement. Maintenant il suffit de
...
C'est ce fichier uniquement qui sert d'entrée pour l'outil \texttt{ocamldoc}.
