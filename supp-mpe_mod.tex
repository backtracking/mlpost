...
\def\SpecialsgetnormalMPsegmenta
 {
   ...
   \dumb=\myx pt
   \advance\tmp by \dumb
   \global\edef\Specialsmataa{\Specialsrempt\tmp}
   \tmp=\Specialsmatbc pt
   \multiply\tmp by -1
   \dumb=\myy pt
   \advance\tmp by \dumb
   \global\edef\Specialsmatba{\Specialsrempt\tmp}
}
...
\def\SpecialsgetnormalMPsegmentb
 {
   ...
   \dumb=\myx pt
   \advance\tmp by \dumb
   \global\edef\Specialsmatab{\Specialsrempt\tmp}
   \tmp=\Specialsmatbc pt
   \multiply\tmp by -1
   \dumb=\myy pt
   \advance\tmp by \dumb
   \global\edef\Specialsmatbb{\Specialsrempt\tmp}
}

% Replace the true output function by a filter. It replace the square
% by the corresponding image

\let\donormalPDFcode\PDFcode
\def\normalPDFcode#1{\donormalPDFcode{#1}}
\def\PDFcode#1{
         \message{[\the\Specialstrigimage]}
