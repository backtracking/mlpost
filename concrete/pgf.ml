...
module PGF = struct
  ...
  let curveto fmt p1 p2 p3 =
    fprintf fmt "\\pgfpathcurveto{%a}{%a}{%a}"
      point p1
      point p2
      point p3
  ...
  let transform fmt t =
    if t = Matrix.identity then () else
      fprintf fmt "\\pgflowlevel{\\pgftransformcm{%a}{%a}{%a}{%a}{%a}}"
        float t.xx float t.yx float t.xy float t.yy point {x=t.x0;y=t.y0}
  ...
  let glyph fmt p cl font =
    fprintf fmt "\\pgftext[base,left,at={%a}]{{\\font\\mlpostfont=%s at %apt {\\mlpostfont %a}}}"
      point p
  ...
  let rectangle fmt p w h =
    fprintf fmt "\\pgfpathrectangle{%a}{%a} %t"
      point p
      point {x=w;y=h}
      fill
...
let in_context fmt f = fprintf fmt "@[<v>%t@, @[<v>%t@]@,%t@]" PGF.gsave f PGF.grestore
...
let fill_rect fmt trans i x y w h =
  ...
  in_context fmt (fun _ ->
    fprintf fmt "%a@ %a@ %a"
      PGF.transform trans
      PGF.dvi_color i.Dviinterp.color
      PGF.rectanglep (p,w,h)
  )
...
let draw_char fmt trans text =
  ...
  in_context fmt (fun _ ->
    fprintf fmt "%a@ %a@ %a"
      PGF.transform trans
      PGF.dvi_color text.Dviinterp.tex_info.Dviinterp.color
      (PGF.glyphp p) (text.tex_string, text.tex_font)
  )
...
let tex_cmd fmt trans c =
  ...
  | Dviinterp.Draw_text_type1 { c_glyph; c_font; c_pos; c_info } -> assert false
...
let curveto fmt s =
  ...
  if sa = sb && sc = sd
  then PGF.lineto fmt sd
  else PGF.curveto fmt sb sc sd
...
let path =
  let rec path fmt = function
    | S.Path p ->
        ...
        | (x::_) as l ->
            fprintf fmt "%a@ %a"
              PGF.moveto (Spline.left_point x)
              (list space curveto) l
        end ;
        if p.S.cycle then begin fprintf fmt " %t" PGF.close_path end
    | S.Point p ->
        fprintf fmt "%a@ %a"
          PGF.moveto p
          PGF.lineto p in
...
let rec picture fmt p =
  ...
  | P.ExternalImage (f,m,t) ->
    in_context fmt (fun _ ->
        fprintf fmt "%a@ \\pgftext{\\includegraphics{%s}}"
          PGF.transform t f
      )
...
let draw fmt x =
  ...
  fprintf fmt "%%%%Creator: Mlpost %s@." Version.version;
  in_context fmt (fun _ ->
      fprintf fmt "%a@ %a@ %a@ %a"
        PGF.setlinewidth (P.default_line_size /.2.)
        PGF.setlinecap PGF.RoundCap
