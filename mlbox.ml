...
let circle c pic = 
  ...
  let dxp = yscaled 0. marginpoint in
  let dyp = xscaled 0. marginpoint in
  let halfxdiff = add (mult 0.5 (sub lr ll)) dxp in
  let halfydiff = add (mult 0.5 (sub ul ll)) dyp in
