...
let gray_grid w f =
  ...
  Box.draw b ++
  iter 0 (h-1) (fun i -> iter 0 (w-2) (fun j ->
    if i mod 2 = 0 then
      box_arrow (sub i j) (sub i (j+1))
    else
      box_arrow (sub i (j+1)) (sub i j)
  )) ++
  iter 0 (h-2) (fun i ->
    if i mod 2 = 0 then
      smart_box_arrow [Rightn (bp 25.); Down; Left] 
	(sub i (w-1)) (sub (i+1) (w-1))
    else
      smart_box_arrow [Leftn (bp 25.); Down; Right] 
	(sub i 0) (sub (i+1) 0)
  )
