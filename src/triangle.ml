...
let create ?brush ?(stroke=Some black) ?pen ?dash ?fill
    ?left ?right ?width ~depth () =
  let width = match width with None -> depth | Some w -> w in
  let xl, xr = match left, right with
    | None, None -> -.0.5, 0.5
    | Some l, None -> -. l, 1. -. l
    | None, Some r -> r -. 1., r
    | Some l, Some r -> let s = l +. r in -. l /. s, r /. s
  in
  let xl = xl *. width in
  let xr = xr *. width in
...
let tex_root_label ?pos s t =
  root_label ?pos (Picture.tex s) t
...
let label ?x ?(depth=0.7) ?pos label t =
  Box.add_post_draw
    (fun t -> let p = x_depth ?x ~depth t in Command.label ?pos label p) t

let tex_label ?x ?depth ?pos s t =
  label ?x ?depth ?pos (Picture.tex s) t
