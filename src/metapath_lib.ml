        ...
        begin
          match p with
            ...
            |{right=Curl (tp,cc);link={left=lt; _}; _} ->
                ...
                vv.(0) <- -. (psik.(1) *. uu.(0));
                ww.(0) <- 0.
            |{right=Open _; _} -> uu.(0) <- 0.;vv.(0) <- 0.;ww.(0) <- 1.
            | _ ->
...
let make_choices knots =
  ...
  while !p != knots do
    (match !p with
      | ({coord=coord;right=(Given _|Curl _|Open _);link=q; _} as k)
        when coord == q.coord ->
          if debug then
  ...
  let h =
    (let rec aux = function
       ...
       | {right= (Endpoint | Endcycle _ | Explicit _ | Curl _ | Given _); _}
          as h -> h
       | {left = Open t; _} as h when h==knots -> knots.left <- Endcycle t; knots
       | {link=h; _} -> aux h in
     aux knots) in
  if debug then
...
(let rec aux = function
   ...
   | p -> let n,q =
       (let rec search_q n = function
          |{left=Open _;right=Open _;link=q; _} -> search_q (n+1) q
          | q -> (n,q) in
...
let print_option f fmt = function
 ...
 let kmeta_to_path ?cycle meta =
   ...
   | {right = Explicit sb;coord = sa;
      link={left = Explicit sc;coord = sd; _} as s; _} ->
